...
def compare(user_score, computer_score):
    if user_score == computer_score:
        ...
        return "Lose, opppnent has Blackjack"
    elif user_score == 0:
...
def play_game():
    ...
    # then the user loses. If the user has a blackjack (0), then the user wins. If the user_score is over 21, then the user loses. If the computer_score is over 21, then the computer loses. If none of the above, then the player with the highest score wins.
...
while input("Do you want to play Blackjack? 'Y' or 'N' ?: ").casefold() == 'y':
    click.clear()
    play_game()
